...
class QLearningAgent(ReinforcementAgent):
    ...
    DESCRIPTION: Uses the formula given in class for update function
    """
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.qValues[(state, action)] = ((1 - alpha) * (self.qValues[(state, action)])) + \
            (alpha * sample)
    ...
    def getPolicy(self, state):
        ...
        policy = [action for action in legalActions if maxValue == self.getQValue(state, action)]
    ...
    def getAction(self, state):
        ...
        if probability.flipCoin(1 - self.getEpsilon()):
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        # Q(s, a) = sum( f(s, a) * w )
        Qvalue = sum([feats[key] * self.weights[key] for key in feats])
